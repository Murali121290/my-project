...
class FigureTableHighlighter:
    ...
    def apply_highlighting_to_docx(
        ...
        missing_captions = self._detect_missing_captions(dict_types, caption_types)
    ...
    def _detect_missing_captions(
        self,
        dict_types: Dict[str, Any],
        caption_types: List[str]
    ) -> List[Dict[str, Any]]:
        """
        Detect citations without corresponding captions.

        Returns:
            List of dicts: {"element": "Figure", "id": "Figure 1", "page": 5, ...}
        """
        missing = []
