...
# Named tuple returned by match_citation (BUG-01/02 UPGRADE)
...
# ── Regex patterns ───────────────────────────────────────────────────────────
# BUG-06: added re.DOTALL to _RE_PAREN
# FIX: allow optional ', p. N' / ', pp. N-M' tail after year so locator
# cites like (Scheiman, 2024, p. 145) and (Deemer et al., 2017, p. 1514)
...
# BUG-05: extended initial-exclusion regex to cover hyphenated & multi-dot initials
_INITIAL_ONLY_RE = re.compile(r'^([A-Z]\.?\s*-?\s*)+[A-Z]?\.?$')

# BUG-13: org-name heuristic for ApaFixer._comma guard
...
def _para_label(idx: int, para_offset: int = 0) -> str:
    """QA-01: Return human-readable label for a paragraph index."""
    if idx < 0:
        return "?"
...
def _count_authors(full: str) -> int:
    ...
    # BUG-04b fix: normalise & and 'and' to ', ' before comma-aware split
    # so 'Smith, J., Jones, A. & Brown, C.' correctly yields 3 authors
    ...
    while i < len(raw_parts):
        ...
        if not part:
            i += 1
            continue
        # If next token looks like initials ('J.' / 'J.-P.'), it belongs to this surname
        if (i + 1 < len(raw_parts)
                and re.match(r'^[A-Z]\.', raw_parts[i + 1].strip())):
            i += 1  # consume the initials token
        parts.append(part)
        i += 1
...
def _is_org_match(cite_auth: str, bib_auth: str) -> bool:
    ...
    def match_part(cp):
        cp_norm = _norm(cp)
        # BUG-03 fix: use word-boundary check, not substring, to prevent
        # "roberts" matching inside "robertson"
...
class ApaFixer:
    ...
    def fix(text: str) -> Tuple[str, List[Dict]]:
        ...
        def _comma(m):
            # BUG-13: extended guard for org names
        ...
        def _paren_and(m):
            # Guard against organizations that use 'and' natively
...
class CitationExtractor:
    ...
    def extract(text: str) -> List[Dict]:
        ...
        for m in _RE_PAREN.finditer(text):
            ...
            block: List[Dict] = []
            for seg in segs:
                # FIX: extract locator FIRST, then strip it so _RE_CITE_UNIT
                # sees "Scheiman, 2024" not "Scheiman, 2024, p. 145"
...
class GrobidClient:
    ...
    GROBID (https://github.com/kermitt2/grobid) is a machine-learning service
    that parses raw reference strings into structured TEI XML. It handles edge
    cases the hand-rolled APA regex cannot: chapter-in-book entries, corporate
    authors, hyphenated surnames, translated titles, DOI-only refs, etc.

    Usage
    -----
    client = GrobidClient("http://localhost:8070")   # your GROBID instance
    parsed = client.parse_references(["Smith, J. (2020). A book. Publisher.",
                                      "Jones et al. (2019). ..."])
    # Returns list of dicts with keys: authors, year, title, is_org
    # Falls back to None on any error so callers can use regex fallback.

    Architecture
    ------------
    Called from BibliographyParser.parse_entry() as a first-pass NLP stage.
    When GROBID is unavailable (not configured, timeout, 4xx/5xx), the method
    returns None and the existing regex parser runs as normal.  No exceptions
    propagate — the validator never fails because of GROBID.

    Batch API
    ---------
    parse_references() sends all raw strings in one HTTP call using the
    'citations' field (newline-separated), which is more efficient than one
    call per entry.  CitationProcessor._parse_bibliography() collects the
    full bibliography list and calls GrobidClient.batch() once, then each
    entry picks up its pre-parsed result by list position.

    TEI XML schema (GROBID output)
    --------------------------------
    <biblStruct xml:id="bN">
      <analytic>            ← journal article / book chapter
        <author><persName><surname>…</surname><forename>…</forename></persName></author>
        <title level="a">…</title>
      </analytic>
      <monogr>              ← always present; book / journal container
        <author>…</author>  ← present if monograph (book / report)
        <orgName>…</orgName>← present if corporate/org author
        <title level="m">…</title>  ← book title
        <title level="j">…</title>  ← journal title
        <imprint>
          <date type="published" when="YYYY"/>
          <publisher>…</publisher>
        </imprint>
      </monogr>
    </biblStruct>
    ...
    def is_alive(self) -> bool:
        """Return True if the GROBID service responds to /api/isalive."""
        if not _GROBID_DEPS:
            return False
        try:
    ...
    def parse_references(self, raw_strings: List[str]
        ...
        """
        Send a list of raw reference strings to GROBID in one batch call.

        Returns a list of the same length as *raw_strings*, where each element
        is either a dict with keys::

            authors     : str   – APA-style author field, e.g. "Smith, J., & Jones, A."
            full_author : str   – same as authors (alias expected by BibliographyParser)
            year        : str   – 4-digit year or 'n.d.'
            title       : str   – primary title of the work
            is_org      : bool  – True if the first author is a corporate body
            source      : str   – "grobid"

        …or None if GROBID could not parse that individual entry.

        Returns None (not a list) if the HTTP call itself fails entirely, so
        callers can distinguish "service down" from "entry unparseable".
        """
        if not _GROBID_DEPS:
            return None
        if not raw_strings:
    ...
    def _parse_tei(self, xml_bytes: bytes,
        ...
        """Parse GROBID's TEI XML response into structured dicts."""
        NS  = {"tei": self._TEI_NS}
        try:
    ...
    def _extract_bib(self, bib, NS: dict) -> Optional[Dict]:
        """Extract one <biblStruct> into a BibliographyParser-compatible dict."""

        # ── Authors ───────────────────────────────────────────────────────────
        # Try analytic first (journal articles), then monogr (books/reports).
        author_nodes = (bib.findall(".//tei:analytic/tei:author", NS)
                        or bib.findall(".//tei:monogr/tei:author", NS)
                        or bib.findall(".//tei:author", NS))

        # Corporate / org author?
        ...
        for an in author_nodes:
            ...
            if pn is not None:
                ...
                if surname:
                    # Reconstruct APA "Surname, F. M." style
        ...
        # Fallback: bare <orgName> outside <author>
        ...
        for date_el in bib.findall(".//tei:date", NS):
            ...
            if re.match(r"^(19|20)\d{2}", when):
                ...
                break
        if not year:
            # Last-resort: scan text of the whole element for a 4-digit year
    ...
    def _to_initials(forename: str) -> str:
        """
        Convert a GROBID forename string to APA initials.
        'Anna Maria' → 'A. M.'
        'A M'        → 'A. M.'
        'Jean-Pierre'→ 'J.-P.'
        """
        ...
        # Re-join with hyphen if original had a hyphen
        if "-" in forename:
            return "-".join(inits)
        return " ".join(inits)
...
class BibliographyParser:
    ...
    # GROBID pre-parsed cache: populated by batch_parse() before parse_entry()
    # is called.  Maps raw string → grobid dict (or None if unparseable).
    ...
    def batch_parse(cls, raw_strings: List[str],
        ...
        """
        Call GROBID once for all bibliography entries and populate the cache.
        Must be called before the first parse_entry() call in a session.
        If *grobid* is None or the call fails, the cache stays empty and
        parse_entry() falls back to regex for every entry.
        """
        ...
        if results is None:
            return   # service unavailable — regex fallback for all
        for raw, parsed in zip(raw_strings, results):
            cls._grobid_cache[raw] = parsed

    @classmethod
    def parse_entry(cls, raw: str) -> Optional[Dict]:
        # ── Stage 1: GROBID NLP (if cache populated by batch_parse()) ────────
        g = cls._grobid_cache.get(raw)
        if g is not None:
            # GROBID gave us structured author/year — build the full entry dict
            # exactly as the regex path does, reusing _display / _count_authors.
        ...
        # ── Stage 2: Regex fallback (original logic) ──────────────────────────
        ...
        if m:
            ...
            if not ym:
                return None
            year = ym.group(1)
            # QA-03 / BUG-07: smarter AMA author extraction
    ...
    def _extract_ama_authors(raw: str) -> str:
        """
        QA-03: Robust AMA author extraction that handles 'Smith Jr.' and
        multi-period names by finding the first likely end-of-author-block.
        """
        # Try to find a year or title-start (capital word after period+space)
        # and take everything before it as the author block.
        m = re.match(r'^(.*?)\.\s+[A-Z"«\[]', raw)
        if m:
            return m.group(1)
        # Fallback: first segment before a year
    ...
    def _display(full: str, is_org: bool = False) -> str:
        ...
        for part in parts:
            ...
            if re.search(r'\bet\s+al\b', part, re.IGNORECASE):
                ...
                if not part:
                    continue
            if _INITIAL_ONLY_RE.match(part):  # BUG-05: extended regex
                continue
            sn.append(part)
...
def _score_candidate(cite_norm: str, cite_first: str, cite_year: str,
    ...
    """
    Composite match score used by BUG-01/02 fix.
    Name similarity (0–1) + year bonus (0 or 2).
    Year match weighted 2× so a correct-year weak-name hit beats
    wrong-year perfect-name hit when year_bonus=2 > name_sim=0.9 scenario
    is resolved correctly in practice.
    """
    name_sim = difflib.SequenceMatcher(None, cite_norm, ref_norm).ratio()
    year_ok  = _strip_suffix(cite_year) == _strip_suffix(ref_year)
    return name_sim + (2.0 if year_ok else 0.0)
...
def match_citation(
    ...
    """
    BUG-01 + BUG-02 rewrite: every candidate is scored; the highest-scoring
    candidate wins.  Score = name_similarity + 2×year_match.

    Returns MatchResult(key, match_type, score).
    Callers that unpack only (key, match_type) continue to work.
    """
    cn = _norm(cite_author)
    cf = _first_surname(cite_author)
    cb = _strip_suffix(cite_year)
    cu = cite_author.strip().upper()
    ...
    for key, ref in bibliography.items():
        ...
        if cn == rn:
            ...
            score = _score_candidate(cn, cf, cite_year, rn, rf, ry)
              ...
              (cf == rf or
               # BUG-03: short names require exact first-surname
            ...
            if ratio >= FUZZY_THRESHOLD:
                ...
                score = ratio + (2.0 if year_ok else 0.0)
            else:
    ...
    if best_mt in ("year_mismatch",) and cb == rb and cite_year != ry:
        # Same base year, different suffix  → suffix_mismatch or suffix_ambiguous
...
def check_etal_enforcement(cite_author: str, bib: Dict) -> Optional[str]:
    ...
    if n < ET_AL_MIN and has:
        return (f"et al. INCORRECT: entry has {n} author(s) ('{bib['display']}') "
                "— list all names.")
    # BUG-12: period-check only fires when n >= ET_AL_MIN and has == True
...
def apply_highlight_by_span(para, char_start: int, char_end: int,
    ...
    """Apply highlight and character style precisely to the character range
    [char_start, char_end) in *para*, splitting boundary runs as needed.

    char_start/char_end are para.runs-based offsets (NOT _full_text offsets,
    which include <w:ins> content). Callers must compute positions from
    runs-only text: ``"".join(r.text for r in para.runs)``.
    """
    ...
    while attempts < 20:
        attempts += 1
        runs   = list(para.runs)
        # Build runs-only text for inner_txt extraction
        ...
        for i, run in enumerate(runs):
            ...
            if fully_inside:
                ...
                # Boundary run — use runs-only text for inner_txt
...
def _full_text(p):
    """Return the full visible text of paragraph *p*.

    Reads <w:r><w:t> directly AND <w:ins><w:r><w:t> (tracked insertions),
    so that citations placed inside tracked-change markup are still findable
    by apply_style_to_text / safe_splice after a preceding tracked_replace
    has consumed earlier runs in the same paragraph.

    Ignores <w:del> content — deleted text is not visible.
    """
    ...
    for child in p._p:
        ...
        if tag == _qn('w:r'):
            ...
            for r in child.findall(_qn('w:r')):
                ...
                if t is not None and t.text:
                    parts.append(t.text)
        # w:del children intentionally skipped
    return "".join(parts)
...
_tc_rev_id = 0   # global revision-ID counter; incremented before each use
...
def _tc_ins(text: str, base_rPr=None) -> "OxmlElement":
    """Return a <w:ins> wrapping a single run with *text*."""
    import copy
...
def _tc_del(text: str, base_rPr=None) -> "OxmlElement":
    """Return a <w:del> wrapping a single run with *text* as <w:delText>."""
    import copy
    d = OxmlElement('w:del')
...
def _tc_rpr_change(run_el, new_highlight=None, new_style_id=None):
    """Add <w:rPrChange> to *run_el* to track a formatting-only change.

    Records the run's *original* rPr inside <w:rPrChange> so Word shows the
    formatting difference as a tracked revision.
    """
    import copy
    rpr = run_el.get_or_add_rPr()

    # Snapshot original rPr (before we mutate it) for <w:rPrChange>
    orig_rpr = OxmlElement('w:rPr')
    for child in list(rpr):
        if child.tag != qn('w:rPrChange'):   # don't nest
...
def tracked_replace(para, old_text: str, new_text: str,
    ...
    so Word shows the substitution as an accepted/rejected revision.
    Returns True if the replacement was made.

    IMPORTANT: safe_splice operates on para.runs offsets (skips <w:ins>/<w:del>
    nodes), so we must locate old_text using runs-only text, not _full_text which
    includes <w:ins> content and would give misaligned character offsets.
    """
    import copy

    # Use para.runs-only text for position calculation (safe_splice uses same)
    ...
    # Isolate the target span into a single run via safe_splice, then replace
    # that run with <w:del>+<w:ins> pair in the XML.
    spliced = safe_splice(para, pos, end, old_text, None, None)
    if spliced is None:
        return False

    # spliced is the CT_R element for the isolated run
    base_rPr = (copy.deepcopy(spliced.rPr)
                if hasattr(spliced, 'rPr') and spliced.rPr is not None
                else None)

    # Build new rPr with highlight + style for the insertion
...
def safe_splice(para, start, end, new_text, highlight, style):
    ...
    if s_r_idx == e_r_idx:
        # BUG-07: guard against empty pre — remove zero-length artefact
        if pre:
            ...
            # pre is empty: replace the run content directly
            runs[s_r_idx].text = ""
            pe.insert(ai + 1, st._r)
            if post:
                post_r = OxmlElement('w:r')
...
def isolate_target_run(p, txt):
    """Isolate *txt* in *p* into a single run via safe_splice.

    Uses para.runs-only text for position calculation — safe_splice operates
    on para.runs offsets and skips <w:ins>/<w:del> nodes in the XML.
    Returns the CT_R element of the spliced run, or None if not found.
    """
    if not txt:
        return None
    # Use runs-only text — safe_splice skips <w:ins>/<w:del>
...
def apply_style_to_text(p, txt, hl):
    ...
    res    = None
    while True:
        # Use runs-only text — safe_splice operates on para.runs offsets
        runs_text = "".join(r.text for r in p.runs)
    ...
    # Span-based fallback for multi-run text
...
def sort_citation_block(para, block_raw: str, highlight) -> tuple:
    """Sort a multi-citation parenthetical block alphabetically in-place.

    Returns (success: bool, already_styled: bool).
    - success=True  when the block was found and rewritten
    - already_styled=True when tracked_replace put the sorted text into a
      <w:ins> run with highlight+style already applied — caller must NOT
      re-apply highlight via apply_highlight_by_span or it will create a
      duplicate run outside the <w:ins>
    """
    if not block_raw:
        return False, False
...
def _get_comments_part(doc):
    """Return the comments part for *doc*, creating it if absent.

    QA-02 / FIX-20: The previous implementation used ``package._parts``
    (a dict) which no longer exists in python-docx >= 1.0.  Parts are now
    discovered by traversing the relationships graph via ``iter_parts()``,
    and new parts are registered simply by calling ``relate_to`` on the
    correct source part — no manual dict manipulation required.
    """
    try:
        ...
        # 1. Already related directly from the document part?
        for rel in doc.part.rels.values():
            if rel.reltype == REL:
                return rel.target_part

        # 2. Exists somewhere in the package graph but not yet related?
        #    (FIX-20: use iter_parts() — _parts dict no longer exists)
        for p in doc.part.package.iter_parts():
            if str(getattr(p, "partname", "")) == "/word/comments.xml":
                doc.part.relate_to(p, REL)
                return p

        # 3. Create a brand-new, empty comments part and wire it in.
        ...
        # FIX-20: relate_to automatically registers the part in the rels
        # graph — no need to insert into a _parts dict.
...
def insert_comment(doc, para, text, target_run=None, target_text=None):
    """Anchor a comment to a run in *para* using the native python-docx API.

    FIX-22: Replaced hand-rolled XML with doc.add_comment() to fix schema
    errors (missing w:date, w:pStyle CommentText, w:annotationRef) that
    caused Word to reject the file with "XML data is invalid".

    Extended: when target_text lives inside a <w:ins> node (from tracked_replace),
    isolate_target_run (which uses safe_splice / para.runs) won't find it.
    We search <w:ins> children directly in that case.
    """
    try:
        ...
        def _find_in_ins(search_txt):
            """Return the first <w:r> inside any <w:ins> whose <w:t> contains search_txt."""
            if not search_txt:
                return None
            sl = search_txt.lower()
        ...
        # Resolve target run — try plain runs first, then w:ins runs
        if target_run is None and target_text is not None:
            ...
            # If still not found, search inside <w:ins> nodes
            if target_run is None:
        ...
        # Fallback: first run in paragraph
        if target_run is None:
            target_run = para.runs[0]._r if para.runs else None
        if target_run is None:
            return False

        # safe_splice returns a raw CT_R lxml element; wrap to Run if needed
        if isinstance(target_run, _Run):
            run_obj = target_run
        else:
            run_obj = _Run(target_run, para)

        # Native API produces fully schema-valid comments.xml
        doc.add_comment(
            runs=run_obj,
            text=text,
    ...
    def summary(self) -> str:
        ...
        for itype, heading in _SEC:
            ...
            if items:
                lines += ["", heading, "-" * 52]
                for item in items:
                    # QA-01: use _para_label for readable table references
...
class CitationProcessor:
    ...
    def __init__(
        ...
        # BUG-11: store para_offset for virtual table-cell idx labelling
        self._para_offset: int = 0
        # Block highlight accumulator: blk_raw → {para, para_idx, all_ok, has_issue}
        # Populated during _validate_citation; flushed by _apply_block_highlights()
        # called at end of each paragraph so blocks are coloured once as a unit.
    ...
    def _parse_bibliography(self):
        # ── Collect raw bib strings first ────────────────────────────────────
        raw_entries: List[Tuple[int, str]] = []  # (para_idx, text)
        in_bib = False
        ...
        # ── GROBID batch parse (single HTTP call for entire bibliography) ─────
        # Populates BibliographyParser._grobid_cache so that parse_entry()
        # can use NLP-parsed fields instead of regex for each entry.
        ...
        # ── Per-entry parsing (uses GROBID cache when available) ─────────────
    ...
    def _process_body(self):
        ...
        # Tables — BUG-11: unique virtual_idx per cell.
        # FIX-17: also scan nested tables (tables inside table cells).
        # FIX-23: diagonal/complex merges caused citation cells to be skipped.
        #   root cause: iterating row.cells returns duplicates for merged cells
        #   (the same _tc appears at multiple grid positions). When a header row
        #   shares a _tc with a later data-row cell via a staircase merge, the
        #   data-row cell gets added to seen_cell_ids during the header-row pass
        #   and is then skipped — so citations in those cells are never extracted.
        #   Fix: walk the raw XML <w:tr>/<w:tc> children directly; each physical
        #   <w:tc> element appears exactly once in the XML regardless of how many
        #   virtual grid positions it spans.
        all_seen_tc: set = set()    # global dedup across the whole document

        def _iter_unique_tcs(table):
            """Yield each physical <w:tc> element exactly once, walking raw XML.
            row.cells / merged-cell grid can return the same _tc multiple times;
            iterating <w:tr>/<w:tc> children avoids every duplicate."""
            from docx.oxml.ns import qn as _qn
        ...
        for tbl_idx, table in enumerate(self.doc.tables):
            # Skip tables whose cells were already visited via a parent cell
            from docx.oxml.ns import qn as _qn
            first_tcs = {id(tc)
    ...
    def _process_para(self, para, idx: int, txt: str,
        ...
        """Shared body/table-cell processing logic."""
        ...
        # Flush block highlights after all cites in this para are resolved
    ...
    def _validate_citation(self, cite, para_idx, para):
        ...
        if is_new_block:
            ...
            if not cite.get("block_order_ok", True):
                # Auto-sort the block text in the document
                sorted_ok, already_styled = sort_citation_block(para, blk_raw, YELLOW)
                # Compute sorted form for the comment message
                inner = blk_raw.strip("()")
                ...
                # Register in pending — already_styled=True means tracked_replace
                # embedded the colour inside <w:ins>; _apply_block_highlights skips it
                ...
                # Already in order — register pending for green highlight
        ...
        # Closure: mark the pending block for this member as needing yellow
        ...
        if mt in ("exact", "smart", "org_abbrev"):
            ...
            # For block members: the block's colour is managed by
            # _apply_block_highlights. Per-cite text ops would try to find
            # the individual string inside <w:ins> (after sort) via _full_text,
            # then safe_splice would fail (para.runs skips <w:ins>), causing
            ...
            self._add_issue(
                ...
                f"changed to \"{auth}, {ref['year']}\" to match with the reference list. "
                "Please confirm.")
    ...
    def _apply_block_highlights(self):
        """Flush pending block highlights after all cites in a paragraph are validated.

        For blocks handled by tracked_replace (already_styled=True), the
        highlight/style is already inside the <w:ins> run's rPr.  We only need
        to update the colour if all members matched cleanly (YELLOW→GREEN).

        For in-order blocks (was_sorted=False) the text is in plain runs; we
        apply highlight+style via apply_highlight_by_span using para.runs-based
        offsets (NOT _full_text offsets which would include <w:ins> content and
        cause misaligned splices).
        """
        from docx.oxml.ns import qn as _qn
        ...
        for blk_key, pb in self._pending_blocks.items():
            ...
            if pb.get("already_styled"):
                # The block lives inside a <w:ins> node already carrying
                # YELLOW highlight + cite_bib style from tracked_replace.
                # If all members matched cleanly, upgrade the colour to GREEN.
                if pb["all_green"]:
                    for child in para._p:
                ...
                # Already styled — do NOT call apply_highlight_by_span
                continue

            # In-order block or plain-splice fallback: highlight in plain runs.
            # Use para.runs-based offset (not _full_text which includes <w:ins>)
            runs_text = "".join(r.text for r in para.runs)
    ...
    def _flag_unused(self):
        # BUG-09: dedup by (norm_author, year) not raw string
        ...
        for k, ref in self.bibliography.items():
            ...
            # FIX: use full ref text as target_text so insert_comment can
            # anchor to the actual run, and highlight the whole entry yellow.
            ...
            # FIX: highlight the entire bibliography entry in yellow, and
            # capture the returned run so insert_comment can anchor directly
    ...
    def _insert_comments(self):
        for iss in self._issues:
            ...
            if p is None:
                continue
            # For unused-ref issues, apply_style_to_text has already isolated
            # the run and returned it — pass it directly to avoid re-splicing.
            hr = iss.get("highlight_run")
            if hr is not None:
                # FIX-21: apply_style_to_text → safe_splice returns st._r,
                # a raw CT_R lxml element. hr is already that element — do NOT
                # call hr._r (CT_R has no ._r attribute; that belongs to Run).
    ...
    def run(self) -> ValidationReport:
        ...
        # Record how many bib entries were parsed by GROBID vs regex
        grobid_count = sum(
            1 for e in self._bib_ordered
            if e.get("grobid_title") is not None   # set only on GROBID path
        )
        if grobid_count:
            self._stats["grobid_parsed"] = grobid_count

        # BUG-10: include multi_year in total_cites
