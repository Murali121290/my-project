...
class CitationAnalyzer:
    ...
    def _setup_regex_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns['range'] = re.compile(
            r'(?:\(|\b)(Figures?|Figs?\.?|Tables?|Tabs?\.?|Boxes?|Exhibits?|Appendices?|Case\s+Studies?)\.?\s+([0-9]+(?:[\.\-][0-9]+)*)([A-Za-z]?)(?:\s+(?:to|through)\s+|\s*[\u2013\u2014]\s*|[ \t]+-[ \t]+)([0-9]+(?:[\.\-][0-9]+)*)([A-Za-z]?)(?:\)|\b)',
            re.IGNORECASE
        )
        ...
        patterns['list'] = re.compile(
            r'(?:\(|\b)(Figures?|Figs?\.?|Tables?|Tabs?\.?|Boxes?|Exhibits?|Appendices?|Case\s+Studies?)\.?\s+([0-9]+(?:[\.\-][0-9]+)*[A-Za-z]?(?:\s*,\s*[0-9]+(?:[\.\-][0-9]+)*[A-Za-z]?)*(?:\s*,?\s+(?:and|&)\s+[0-9]+(?:[\.\-][0-9]+)*[A-Za-z]?))(?:\)|\b)',
            re.IGNORECASE
        )
...
def build_unnumbered_tab_html(unnumbered_counts: dict) -> str:
    ...
    rows = [
        ...
        ("Tables",    unnumbered_counts.get("unnumbered_tables",  0),
         _with_pages("Tables without caption appear on pages: ",
                     unnumbered_counts.get("unnumbered_table_pages", []))),
        ("Boxes",     unnumbered_counts.get("unnumbered_boxes",   0),
         _with_pages("Boxes without caption appear on pages: ",
                     unnumbered_counts.get("unnumbered_box_pages", []))),
...
def extract_with_docx(doc_path: str, doc: Optional[Any] = None):
    ...
    if has_lrpb:
        ...
        for p in doc.paragraphs:
            ...
            text = "".join(run.text for run in p.runs if not run.font.superscript).strip()
            if not text:
                continue
    ...
    for tbl_idx, table in enumerate(doc.tables):
        ...
        for cell in first_row.cells:
            for cp in cell.paragraphs:
                cell_text = "".join(run.text for run in cp.runs if not run.font.superscript).strip()
                if not cell_text:
                    continue
                try:
...
def generate_multilingual_html(doc_path: str,
    ...
    for i, p in enumerate(doc.paragraphs):
        ...
        for match in keyword_pattern.finditer(text):
            ...
            keyword_map[key].append((page, text.strip()))

        for char in text:
            code = ord(char)
            for lang, low, high in multilingual_ranges:
                if low <= code <= high:
                    multilingual_map[lang].append((page, text.strip()))
                    break

    def _make_table(title, data, empty_msg):
        t = f"<h3>{title}</h3>"
        t += "<table><thead><tr><th>Type</th><th>Paragraph Text</th><th>Page</th></tr></thead><tbody>"
        if data:
    ...
    html  = _make_table("Keywords", keyword_map, "No keywords found")
    html += _make_table("Multilingual Characters", multilingual_map, "No multilingual characters found")
    return html
...
def extract_chapter_metadata(doc_path: str, doc: Optional[Any] = None):
    ...
    for idx, p in enumerate(paragraphs):
        ...
        if bare_m:
            chapter_number = text
            for look_p in paragraphs[idx + 1:idx + 2]:
